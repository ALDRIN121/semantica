...
- **Validating `sh:class` on a property that declares `rdfs:range` with RDFS entailment on**: RDFS is an entailment rule, not a constraint. When pyshacl runs with `inference="rdfs"`, it infers the range class onto every object of the property, so a `sh:class` constraint on that property can never fail — the report says `conforms: True` on data that does not conform:

    ```python
    from pyshacl import validate

    # ex:notAnItem is a Fish; ex:contains declares rdfs:range ex:Item.
    # shapes: [ sh:path ex:contains ; sh:class ex:Item ]
    for inference in ("none", "rdfs"):
        conforms, _, _ = validate(data, shacl_graph=shapes, inference=inference)
        print(inference, conforms)
    # none  False   <- correct: notAnItem is not an Item
    # rdfs  True    <- the entailment manufactured the type
    ```

    When class membership is the thing under test, run validation without RDFS entailment (`inference="none"`), or express the check as a constraint the entailment cannot satisfy (for example a literal property constraint).
- **Trusting `conforms: True` without checking the inference mode**: an inference-enabled run can hide the exact violations the shapes were written to catch (see above). Record which inference mode validation ran under alongside the result, and re-run shape sets that contain `sh:class` with entailment off before treating a pass as authoritative.
